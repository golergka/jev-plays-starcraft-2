...
async def main():
    ...
    jev=Jev(lambda *a,**k:None,'cargo-continuation-probe',max_calls=6)
    result={'source':str(source),'pairs':[],'method':'First/middle/last occupied Bunker concrete decisions. Same full states and batches; only continue description adds observed passengers and explicit absence of unload command. All alternatives retained, alternating arms. No gameplay or claim of improved survival.'}
    for i,row in enumerate([rows[0],rows[len(rows)//2],rows[-1]]):
        q=copy.deepcopy(row['questions']);f=row['state']['selection_facts']['Bunker']
        q['Bunker']['criteria']['continue'] += (' Issue no new order to this selection; in particular, do not issue an unload command. '
            'Currently observed passengers remain aboard unless existing orders or game events change that: '+json.dumps(f['passengers_by_type'])+'.')
        pair={'loop':row['state'].get('game_loop'),'passengers':f['passengers_by_type'],
              'treatment_description':q['Bunker']['criteria']['continue'],'answers':{}}
        result['pairs'].append(pair)
        for arm in (['baseline','explicit_cargo'] if i%2==0 else ['explicit_cargo','baseline']):
