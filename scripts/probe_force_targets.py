"""Offline task-structure probe: immediate investment versus desired owned counts."""
import asyncio
import json
import sys
...
def target_questions(state):
    ...
    for index,name in enumerate(types):
        ...
        targets=sorted({0,current,current+1,current+3,current+6,current+12})
        questions[f'target_{index}']={'type':'choice',
            'instructions':f'Choose the desired owned count of {name} for the next 2016 game loops to advance the mission objective. Currently observed count: {current}. This is a hypothetical production target, not a prediction or a command. Consider the shared resources, other unit types, losses, and strategic priority. A later allocation decision would select which target to fund and which producer to use. No purchase is implied if resources or legal production are unavailable.',
            'criteria':{f'count_{n}': f'Desired total {name}: {n}. '+('Do not produce this type during the horizon; existing units are retained.' if n==0 else 'Request production only while the observed owned count is below this target; no units are removed if above it. Losses can create new demand until the horizon expires.') for n in targets}}
    return questions
...
async def main():
    ...
    model=Jev(lambda *a,**k:None,'force-target-probe',max_calls=6)
    result={'source':str(source),'method':'First/middle/last investment states with at least300minerals and observed Train capability. Immediate investment versus desired owned-count targets; not equivalent questions or proof of better play. Alternate arm order. No commands.','pairs':[]}
