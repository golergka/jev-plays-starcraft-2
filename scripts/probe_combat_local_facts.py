"""Paired opening-combat probe: put selection facts beside concrete orders."""
import asyncio
import json
import sys
...
async def main():
    ...
    for line in source.read_text().splitlines():
        row = json.loads(line)
        if row['event'] == 'jev' and 'MobileCombat' in row.get('questions', {}):
            rows.append(row)
    if len(rows) < 3:
        raise ValueError('Need three recorded MobileCombat requests')
    selected = rows[:3]
    load_dotenv(Path(__file__).resolve().parents[1]/'.env')
    model = Jev(lambda *a, **k: None, 'combat-local-facts-probe', max_calls=6)
    result = {'source': str(source), 'method': 'First three recorded requests containing MobileCombat. Full original question batch and state in both arms. Treatment repeats each selection factual record verbatim next to its question, with an explicit field reference. No tactical advice, filtering, action changes or game commands. One paired observation per state; variation is not causal proof.', 'pairs': []}
