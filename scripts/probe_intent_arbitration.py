...
async def main():
    ...
    model = Jev(lambda *a, **k: None, 'intent-arbitration-probe', max_calls=6)
    result = {'source':prior['source'], 'intent_source':str(prior_path),
              'method':'Reuse the three exact recorded states and Jev-produced intentions from the prior probe. Fresh baseline versus identical purchase question with Jev goals added to state. All choices retained, including Save. Alternating order, one pair per state. Reused goals are hypothetical, not observations or forced orders. No gameplay commands.', 'pairs':[]}
    for i, old in enumerate(prior['pairs']):
        row = rows[old['time']]
        goals = [{'question':q['instructions'], 'chosen_intent':q['criteria'][old['targets'][key]['choice']]}
        ...
        pair = {'time':old['time'], 'goals':goals}; result['pairs'].append(pair)
