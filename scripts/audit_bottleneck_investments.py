    ...
    for line in source.read_text().splitlines():
        ...
        if e['event'] == 'jev':
            ...
            row = {'loop': loop, 'diagnosis': state.get('jev_bottleneck_diagnosis'),
                   ...
                   'selected_choice': None, 'selected_description': None, 'selection_mode': None}
            ...
            row.update(sampled_choice=e['sampled_choice'], sampled_description=criteria.get(e['sampled_choice']),
                       selected_choice=e['sampled_choice'], selected_description=criteria.get(e['sampled_choice']),
                       selection_mode='sampled_distribution')
        elif e['event'] == 'investment_top_choice' and e.get('loop') in pending:
            row, criteria = pending[e['loop']]
            row.update(selected_choice=e['choice'], selected_description=criteria.get(e['choice']),
                       selection_mode='returned_choice')
...
if __name__ == '__main__':
    ...
    print(json.dumps({'calls': result['calls'], 'cost_usd': result['cost_usd'], 'reviews': [
        {'loop': r['loop'], 'diagnosis': (r['diagnosis'] or {}).get('answer'),
         'top': (r['top_description'] or '')[:80], 'selected': (r['selected_description'] or '')[:80], 'mode':r['selection_mode']}
