...
async def main():
    ...
    model=Jev(lambda *a,**k:None,'order-kind-score-probe',max_calls=9)
    ...
    def family(key):
        ...
        return 'move'
    result={'source':str(source),'method':'First/middle/last exact reconstructed Marine menus; full direct Choice versus independent order-kind Scores followed by exact within-kind Choice. All options retained. Alternate arm order, nine-call ceiling. No game commands.','pairs':[]}
    for index in (0,len(eligible)//2,len(eligible)-1):
        ...
        async def staged():
            questions={key:{'type':'score',
                'instructions':full['instructions']+' Rate the usefulness of this kind of order now; its exact target or destination would be chosen separately. Order kind: '+description,
                ...
                for key,description in descriptions.items() if key in groups}
            pair['scores']=await model.ask(state,questions)
            scores={k:v['score'] for k,v in pair['scores'].items()}
            if set(scores)!=set(groups) or any(not math.isfinite(v) or not 0<=v<=4 for v in scores.values()):raise ValueError('Invalid score response')
            selected=max(scores,key=scores.get)
            pair['kind']={'choice':selected,'tie_rule':'First highest in fixed description order; exploratory probe only, not deployed.'}
