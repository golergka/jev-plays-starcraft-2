...
class VisibleTimerReader:
    def __init__(self, path, *, expected_generation, started_at, max_age=5):
        if expected_generation < 1 or max_age <= 0:
            raise ValueError('Require a positive launch generation and freshness interval')
        self.path = Path(path)
    ...
    def poll(self, now):
        ...
        try:
            if integer(context, 'generation') != self.generation:
                return None
